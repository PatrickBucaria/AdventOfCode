package twentytwentythree.Dec2.ProblemOne


import twentytwentythree.Dec2.Drawing
import twentytwentythree.Dec2.Game
...
fun main() {
    ...
    var result = 0
    games.forEach { game ->
        if (game.isPossibleGivenValues(NUM_RED_CUBES, NUM_GREEN_CUBES, NUM_BLUE_CUBES)) {
            result += game.id
            println("Game with id ${game.id} is POSSIBLE")
        } else {
            println("Game with id ${game.id} is IMPOSSIBLE")
        }
    }
